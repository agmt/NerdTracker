...
You can run NerdTracker in two ways:

## Option 1: Cloud Deployment (Supabase + Cloudflare Workers)
...
**ToDo**: `location-reporter` doesn't work in local deployment:
```
[wrangler:wrn] Miniflare 3 does not currently trigger scheduled Workers automatically.
```
...
### 2. **Setup**
1. Follow `Cloud Deployment` actions to generate keys
2. Create `.env` using `.env.example` and keys generated by `python setup.py`

### 3. **Services will be available at:**
   - Main app: http://localhost:3000
   - Location inserter API: http://localhost:3001
   - Location reporter: Runs as background scheduler
...
### 4. **Useful commands:**
   ...
   docker exec -it -u postgres nerdtracker-postgres-1 psql -d nerdtracker
   ```
